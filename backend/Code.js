...
function getTemplateFolderId() {
  const config = {
    Dev: '102PZ-8yCcL9TEfrQ-JwHlQ_0P3qjiTiZ', // Defaulted to existing dev template
    Prod: '1qXd__JwjnCvnBRtCJMs0AcxkXBIZQt5x', // Defaulted to existing prod template
    Exp: '102PZ-8yCcL9TEfrQ-JwHlQ_0P3qjiTiZ'  // Falling back to Dev template for Exp unless changed
  };
  return config[ENV];
}
...
function fetchCommAttendance(sheetUrl) {
   try {
       ...
       const lastRow = sheet.getLastRow();
       const lastCol = sheet.getLastColumn();
       if (lastRow < 1) return { success: true, participants: [], junctures: [], attendance: {} };
...
function addCommJuncture(sheetUrl, junctureName) {
   ...
   try {
       ...
       sheet.getRange(1, newColIdx).setValue(headerName);
