...
break;
case 'fetchMassPairingData':
result = fetchMassPairingData(payload.sheetUrl);
break;
case 'syncMassPairingUpdates':
result = syncMassPairingUpdates(payload.sheetUrl, payload.updates);
break;
default:
...
function findColByKeywords(headers, key) {
  const keywordMap = {
    'name': ['name', 'traineename', 'volunteername', 'person'],
    'project': ['project', 'proj'],
    'attending': ['attending', 'attendance', 'att'],
    'caregiver': ['caregiver', 'cg'],
    'meetingloc': ['meetinglocation', 'meeting loc', 'meet loc'],
    'dismissalloc': ['dismissallocation', 'dismissal loc', 'dismiss loc'],
    'volpaired': ['vol paired', 'volunteer paired', 'paired'],
    'dietary': ['dietary', 'meal', 'food', 'halal', 'vegetarian'],
    'contact': ['contact', 'phone', 'mobile', 'tel'],
    'remarks': ['remarks', 'note', 'comment', 'obs'],
    'group': ['group', 'grp']
  };
  ...
  for (let kw of keywords) {
    const idx = getColIndex(headers, kw);
    if (idx !== -1) return idx;
  }
  return -1;
}
...
function fetchMassPairingData(sheetUrl) {
try {
const ss = SpreadsheetApp.openByUrl(sheetUrl);
const tSheet = ss.getSheetByName("Trainee Attendance");
const vSheet = ss.getSheetByName("Volunteer Attendance");
...
function syncMassPairingUpdates(sheetUrl, updates) {
...
if (tSheet) {
 ...
 if (tLastRow > 1) {
     const tData = tSheet.getRange(2, 1, tLastRow - 1, tSheet.getLastColumn()).getValues();
     const tHeaders = tSheet.getRange(1, 1, 1, tSheet.getLastColumn()).getValues()[0].map(h => normalizeHeader(h));
     
     let tNameIdx = tHeaders.indexOf("name");
     if (tNameIdx === -1) tNameIdx = tHeaders.indexOf("traineename");
     if (tNameIdx === -1) tNameIdx = 0;
     
     tData.forEach(row => {
         const tName = String(row[tNameIdx]).trim();
         if (tName) {
...
for(let sk of standardKeys) {
  if (findColByKeywords(rawHeaders, sk) === i) {
    foundKey = sk;
    break;
  }
}
...
const isMatch = normHeader === normKey || 
                (findColByKeywords(rawHeaders, 'meetingloc') === i && normKey.includes("meetinglocation")) ||
                (findColByKeywords(rawHeaders, 'dismissalloc') === i && normKey.includes("dismissallocation")) ||
                (findColByKeywords(rawHeaders, 'attending') === i && normKey.includes("attending")) ||
                (findColByKeywords(rawHeaders, 'caregiver') === i && normKey.includes("caregiver"));
if (isMatch) {
  newRow[i] = value;
  if (findColByKeywords(rawHeaders, 'attending') === i) attendingStatus = value.toString().toLowerCase();
  break;
}
}
}
...
if (tFiles.hasNext()) {
   ...
   if (!tFinder.findNext()) {
      ...
      if(projectVal) {
        const tHeaders = tSheet.getRange(1, 1, 1, tSheet.getLastColumn()).getValues()[0];
        const tProjIdx = findColByKeywords(tHeaders, 'project');
        if (tProjIdx > -1) {
...
if (isMatch) {
if (findColByKeywords(rawHeaders, 'project') === i) {
    break;
}
