...
function renderMassPairings() {
   ...
   let trainees = (massPairingData.trainees || []).filter(t => !t.isAttendingN);
   const vols = massPairingData.volunteers || []; // Volunteers already filtered in backend for 'y'
   
   // Calculate unpaired trainees BEFORE filtering
   let unpairedCount = 0;
   trainees.forEach(t => {
   ...
   // If filtered mode is active, only show Unpaired Trainees in the Target list
   if (isFilteredMassPairingMode) {
...
function openQuickPairModal(sourceName, sourceRole) {
   ...
   if (sourceRole === 'VOLUNTEER') {
       ...
       quickPairContext.targetList = (massPairingData.trainees || [])
           .filter(t => !t.isAttendingN && !t.isGoneHome)
           .map(t => t.name);
   } else {
       // Search Volunteers
