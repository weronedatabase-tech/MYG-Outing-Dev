...
function filterNames() { 
 ...
 matches.forEach(name => { 
     ...
     li.oncontextmenu = (e) => handleTraineeLongPress(e, safeName);
     list.appendChild(li); 
 }); 
 if(matches.length === 0) { 
     const li = document.createElement('li'); 
...
function loadVolData(name, isManualNew) { 
 ...
 fieldsDiv.innerHTML = '<div class="text-center"><div class="loader inline-block border-primary"></div></div>'; 
             ...
             if (cleanH.includes("attending")) { 
                 ...
                 <div class="w-full bg-gray-50 dark:bg-black border ${isReadOnly ? 'border-gray-200 dark:border-zinc-800 text-gray-500' : 'border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-white focus-within:border-green-500'} rounded p-2 text-sm shadow-sm">
                     ...
                     <div class="relative">
                         ...
                         <ul id="activeVolsList" class="absolute z-50 w-full bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg mt-1 shadow-xl hidden max-h-40 overflow-y-auto"></ul>
                     </div>
                 </div>
                 `;
