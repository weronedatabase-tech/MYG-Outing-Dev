// ==========================================
// VOLUNTEER & TRAINEE ATTENDANCE UPDATE ENGINE
// Completely re-architected for Zero-Latency and Concurrent Multi-User Safety
// ==========================================

let currentVolSheetUrl = null;
...
function initVolunteerPage() {
    ...
    currentVolSheetUrl = params.get('url');
    
    if(!currentVolSheetUrl) {
        alert("No event selected. Redirecting to home.");
        navigateTo('./');
        return;
    }

    // Zero-Latency Hydration: Instantly render event details using pre-cached CRON data
    const localDataStr = localStorage.getItem('myg_sheetList');
    if(localDataStr) {
        try {
            const parsed = JSON.parse(localDataStr);
            const match = parsed.find(x => x.sheetUrl === currentVolSheetUrl);
            if(match) {
                document.getElementById('displayEventName').innerText = match.displayName || "Unknown Event";
                document.getElementById('displayEventDate').innerText = match.formattedDate || "";
            } else {
                document.getElementById('displayEventName').innerText = "Event Details Not Cached";
                document.getElementById('displayEventDate').innerText = "Verifying with server...";
            }
        } catch(e) {
...
function autoScrollAndFocus(input) { 
...
function submitAttendance(e) { 
    ...
    // STRICT DELTA PAYLOAD COMPUTATION
    // Ensure only specifically modified inputs are pushed to the backend
    // Prevents overriding changes made by concurrent users on identical rows
    let deltaObj = {};
    let isChanged = false;

    // Use normalized header logic to compare correctly
    const norm = (str) => String(str).toLowerCase().replace(/[^a-z0-9]/g, "");

    for (let key in dataObj) {
        const cleanKey = norm(key);
        let foundOriginalVal = undefined;
        
        // Find matching key in original data
        for (let origKey in originalVolData) {
            if (norm(origKey) === cleanKey || 
               (cleanKey.includes("meetinglocation") && norm(origKey).includes("meetinglocation")) ||
               (cleanKey.includes("dismissallocation") && norm(origKey).includes("dismissallocation")) ||
               (cleanKey.includes("attending") && norm(origKey).includes("attending")) ||
               (cleanKey.includes("caregiver") && norm(origKey).includes("caregiver"))) {
                foundOriginalVal = originalVolData[origKey];
                break;
            }
        }
        
        // Compare types strictly. Cast to strings.
        const newVal = String(dataObj[key]).trim();
        const oldVal = foundOriginalVal !== undefined ? String(foundOriginalVal).trim() : "";
    ...
    // Bypass unnecessary network roundtrips
    if (!isChanged && Object.keys(originalVolData).length > 0) {
        showOverlay('success', 'No changes detected.');
        setTimeout(() => { closeOverlay(); }, 1500);
        return;
    }

    // Always inject Name & Project (if applicable) as contextual anchors for the backend row finder
    for (let k in dataObj) {
