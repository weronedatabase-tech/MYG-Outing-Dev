...
function loadVolunteerEvents() {
 ...
 const renderData = (data) => {
     ...
     if (selector) {
         ...
         // Trigger silent pre-fetch as soon as event is selected
         prefetchNamesForSelectedEvent();
     }
...
// Background Silently Fetch Data
function prefetchNamesForSelectedEvent() {
    const url = document.getElementById('volSheetSelector').value;
    if (!url || url === "Select an Event" || url.includes("Loading") || url.includes("Error")) return;
...
// Whenever form resets due to event selection change, pre-fetch names for new target
prefetchNamesForSelectedEvent();
}
...
// Check memory hydration first to execute 0ms DOM load
if (preFetchedNames[requestedType]) {
    renderList(preFetchedNames[requestedType]);
} else {
    // Fallback if user navigated too quickly before pre-fetch could resolve
    ...
    apiCall('getNamesList', { url: url, type: requestedType }).then(res => { 
        ...
        if(res.success) {
            ...
            input.disabled = false;
            input.placeholder = "Error loading names.";
            if(loader) loader.classList.add('hidden');
            list.innerHTML = ""; 
            list.classList.add('hidden'); 
        }
