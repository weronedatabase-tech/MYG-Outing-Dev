...
function initVolunteerApp(url) {
 if (!url) {
     alert("No event selected. Returning to home.");
     window.navigateTo('./');
     return;
 }
 window.currentVolSheetUrl = url;

 const localDataStr = localStorage.getItem('myg_sheetList');
 if (localDataStr) {
     try {
         const parsed = JSON.parse(localDataStr);
         const match = parsed.find(x => x.sheetUrl === url);
         if (match) {
             const eventTitle = document.getElementById('volEventTitle');
             if (eventTitle) eventTitle.innerText = match.displayName;
         }
     } catch(e) {}
 }
...
function loadVolNames(requestedType, reqId) { 
const url = window.currentVolSheetUrl; 
const input = document.getElementById('volNameSearch'); 
const list = document.getElementById('volNameList');
if(!url || !requestedType) return; 
...
if (currentVolTypeRequest !== reqId) return; 
...
fieldsDiv.innerHTML = [1, 2, 3, 4].map(i => `<div class="mb-2"><div class="animate-pulse h-3 bg-gray-200 dark:bg-zinc-700 rounded w-${i%2===0?'1/3':'1/4'} mb-1.5"></div><div class="animate-pulse h-10 bg-gray-100 dark:bg-zinc-800 rounded w-full"></div></div>`).join('');
...
const payload = { sheetUrl: window.currentVolSheetUrl, type: selectedVolType, data: deltaObj, originalData: originalVolData, targetName: target }; 
...
if(res.success) {
   ...
   if(selectedVolType === 'volunteer') { 
       ...
       const reqId = Date.now();
       currentVolTypeRequest = reqId;
       apiCall('getNamesList', { url: url, type: 'volunteer' }).then(r => { if(r.success && currentVolTypeRequest === reqId) allNames = r.names; }); 
   } 
} else {
