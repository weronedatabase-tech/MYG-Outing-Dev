...
function filterActiveVols() {
    ...
    matches.forEach(match => {
        const li = document.createElement('li');
        li.className = "px-3 py-2 bg-slate-800 text-slate-200 border-b border-slate-700 hover:bg-teal-600 hover:text-white cursor-pointer text-xs transition-colors last:border-0";
        li.innerText = match;
...
function updateVolPairedUI() {
    ...
    tagsContainer.innerHTML = currentVolPairedValue.map(v => 
        `<span class="bg-teal-900/50 text-teal-300 border border-teal-700/50 px-2 py-0.5 rounded text-xs flex items-center gap-1">${v} <i class="fa-solid fa-xmark cursor-pointer hover:text-white ml-1" onclick="removeVolPaired('${v.replace(/'/g, "\\'")}')"></i></span>`
    ).join('');
       ...
       if(res.success) { 
           ...
           fieldsToShow.forEach(header => { 
               ...
               if (cleanH.includes("attending")) { 
                   ...
                   inputHtml = `
                   <div class="w-full bg-slate-900 border ${isReadOnly ? 'border-slate-700 text-slate-500' : 'border-slate-600 text-white focus-within:border-green-500'} rounded p-2 text-sm">
                       <div id="volPairedTags" class="flex flex-wrap gap-1 ${currentVolPairedValue.length > 0 ? 'mb-1' : ''}">
                           ${currentVolPairedValue.map(v => `<span class="bg-teal-900/50 text-teal-300 border border-teal-700/50 px-2 py-0.5 rounded text-xs flex items-center gap-1">${v} <i class="fa-solid fa-xmark cursor-pointer hover:text-white ml-1" onclick="removeVolPaired('${v.replace(/'/g, "\\'")}')"></i></span>`).join('')}
                       </div>
                       <input type="hidden" name="${header}" id="volPairedHidden" value="${currentVolPairedValue.join(', ')}">
                       <div class="relative">
                           <input type="text" id="volPairedInput" ${isReadOnly ? 'readonly' : ''} class="w-full bg-transparent outline-none placeholder-slate-500 text-sm" placeholder="Search volunteer..." autocomplete="off" oninput="filterActiveVols()" onfocus="filterActiveVols()">
                           <ul id="activeVolsList" class="absolute z-50 w-full bg-slate-800 border border-slate-600 rounded-lg mt-1 shadow-xl hidden max-h-40 overflow-y-auto"></ul>
                       </div>
                   </div>
                   `;
...
function submitAttendance(e) { 
   ...
   // Ensure any pending volPaired text is added
