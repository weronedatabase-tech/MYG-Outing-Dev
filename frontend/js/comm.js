...
if (commAttData.junctures.length === 0) {
   html = '<option value="">No Junctures Defined</option>';
} else {
...
renderCommAttLists(false);
}

function renderCommAttLists(preserveScroll = false) {
...
let ncScroll = 0, cScroll = 0, ghScroll = 0;
if (preserveScroll) {
    ncScroll = notCheckedList.scrollTop;
    cScroll = checkedList.scrollTop;
    ghScroll = goneHomeList.scrollTop;
}
...
if (preserveScroll) {
    notCheckedList.scrollTop = ncScroll;
    checkedList.scrollTop = cScroll;
    goneHomeList.scrollTop = ghScroll;
} else {
    notCheckedList.scrollTop = 0;
    checkedList.scrollTop = 0;
    goneHomeList.scrollTop = 0;
}
}
...
commAttPollInterval = setInterval(() => {
   ...
   apiCall('fetchCommAttendance', { sheetUrl: currentCommAttSheetUrl }).then(res => {
       if (res.success && !isCommAttSyncing && !hasPendingUpdates()) {
           const oldDataStr = JSON.stringify(commAttData);
           const oldJunctures = JSON.stringify(commAttData.junctures);
           
           let newData = res;
           if(!newData.attendance['__GONE_HOME__']) newData.attendance['__GONE_HOME__'] = {};
           
           const newDataStr = JSON.stringify(newData);
           
           // Stop background UI rebuild if the incoming data is strictly identical
           ...
           if (oldJunctures !== JSON.stringify(commAttData.junctures)) {
               ...
               const searchInput = document.getElementById('commAttSearchInput');
